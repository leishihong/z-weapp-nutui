...
const UElevator: FC<Partial<ElevatorProps> & HTMLAttributes<HTMLDivElement>> = (
	props
) => {
	const {
		height,
	...
	const setListGroup = () => {
		if (listview.current) {
			Taro.createSelectorQuery()
				.selectAll(`.${className} .u-elevator__list__item`)
				.node((el) => {
	...
	return (
		<div className={cls(cx[b()], className)} {...rest}>
			{isSticky && scrollY > 0 ? (
				<div className={cx[b('list__fixed')]}>
						...
						return (
							<div
								className={cls(
									cx[b('list__item')],
									b('list__item'),
									`elevator__item__${idx}`
								)}
								key={idx}
							>
								<div className={cx[b('list__item__code')]}>
									{item[acceptKey]}
								</div>
								<>
									{item.list.map((subitem: ElevatorData) => {
										return (
											<div
												className={cls(cx[b('list__item__name')], {
													[cx[b('list__item__name--highcolor')]]:
			...
			{indexList.length && scrollStart ? (
				<div className={cx[b('code--current')]}>
					{indexList[codeIndex][acceptKey]}
				</div>
			) : null}
			<div className={cx[b('bars')]}>
				<div
					className={cx[b('bars__inner')]}
				...
				>
					{indexList.map((item: any, index: number) => {
						return (
							<div
								className={cls(cx[b('bars__inner__item')], {
									[cx[b('bars__inner__item--active')]]:
...
UElevator.defaultProps = defaultProps;
UElevator.displayName = 'UElevator';

export default UElevator;
