...
export function isElementOf(node?: ReactNode, type?: JSXElementConstructor<any>) {
  if (isValidElement(node)) {
    ...
    if (isFunction(element.type) && !isEmpty(displayName) && endsWith(displayName, get(type, 'displayName'))) {
      return true;
    }
  }
  return false;
}
const useAvatars = (children: ReactNode, shape: AvatarShape, limit: number): [ReactNode[], number] => {
  return useMemo(() => {
...
const Avatar: FC<IAvatarProps> = (props) => {
  ...
  return (
    <View
      ...
      style={sizeStyle}
    >
      {children ? children : <Image lazyLoad src={src} mode={fit} className={cx['avatarImage']} onClick={onAvatar} />}
    </View>
  );
};
...
const AvatarGroup: FC<AvatarGroupProps> = (props) => {
  const { rootCls, className, shape = 'circle', limit = Number.MAX_VALUE, spacing = 'small', total, children } = props;
